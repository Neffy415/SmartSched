...
// Aiven database connection - credentials loaded from .env
const pool = new Pool({
...
async function createFlashcardTables() {
    ...
    try {
        ...
        console.log('\n✅ All flashcard tables created successfully in Aiven!');

    } catch (error) {
        console.error('❌ Error creating flashcard tables:', error.message);
        throw error;
    } finally {
        client.release();
        await pool.end();
...
createFlashcardTables().catch(err => {
    console.error(err);
    process.exit(1);
});
